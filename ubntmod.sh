...
# Getopts
##########
while getopts Bb:D:di:l:IN:n:RK:p:P:r:s:S:t:c:C:f:hU:u:Ww:X:Yy:Zv options
do
        case "${options}"
        in
		B) 
...
### Version 1.0.1 Changes (October 31)
...
latestVersion=$(curl -s ${updateURL} | grep ubntmod- | cut -d"\"" -f2 | cut -d- -f2 | sed 's/\.sh//g' | sort -n | tail -n1)
...
echo "-l "enable/disable",			Enable or Disable Syslog"
...
echo "-B BACUPNAME,	   			Backs up the configuration on the device to the specified file "
